...
pub use error::{NorgError, NorgResult};
...
pub fn parse_norg(content: &str) -> Result<JsValue, JsValue> {
    let ast = rust_norg::parse_tree(content).map_err(|e| NorgError::Parse(format!("{e:?}")))?;
    ...
    js_sys::Reflect::set(&result, &"metadata".into(), &meta)
        .map_err(|_| NorgError::Js("Set metadata failed".to_string()))?;
    js_sys::Reflect::set(&result, &"html".into(), &html.into())
        .map_err(|_| NorgError::Js("Set html failed".to_string()))?;
    let toc = serde_wasm_bindgen::to_value(&toc)
        .map_err(|e| NorgError::Serial(format!("Serialize toc failed: {e}")))?;
    js_sys::Reflect::set(&result, &"toc".into(), &toc)
        .map_err(|_| NorgError::Js("Set toc failed".to_string()))?;
